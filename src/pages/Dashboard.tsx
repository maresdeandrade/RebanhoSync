...
const Dashboard = () => {
  ...
  const pendenciasAgenda =
    useLiveQuery(() =>
      db.state_agenda_itens.where("status").equals("agendado").count(),
    ) || 0;
  const gestosPendentes =
    useLiveQuery(() =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <AlertTriangle
              className={`h-4 w-4 ${gestosPendentes > 0 ? "text-destructive animate-pulse" : "text-emerald-500"}`}
            />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{gestosPendentes}</div>
            <p className="text-xs text-muted-foreground">
              Gestos aguardando envio
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Evolução de Peso (Média)</CardTitle>
          </CardHeader>
