...
export function AnimalDemographicsCard({
  animalRows,
  taxonomyByAnimal,
  sexoFilter,
}: AnimalDemographicsCardProps) {
  const counts = useMemo(() => {
    const map = new Map<string, number>();

    for (const { animal } of animalRows) {
      const taxonomy = taxonomyByAnimal.get(animal.id);
      const descriptor = resolveAnimalVisualDescriptor(
        taxonomy?.display.categoria,
      );
  ...
  const getCount = (profile: string, sexo: string) =>
    counts.get(`${profile}_${sexo}`) || 0;
  ...
  if (isFemale || (!isFemale && !isMale)) {
    displayGroups.push(
      ...
      },
      {
        profile: "vaca-seca",
        label: "Vacas Secas",
        count: getCount("vaca-seca", "F") + getCount("vaca", "F"),
        categoriaLabel: "vaca seca",
        sexo: "F" as const,
      },
      {
        profile: "novilha",
        label: "Novilhas",
        count: getCount("novilha", "F"),
        categoriaLabel: "novilha",
        sexo: "F" as const,
      },
      {
        profile: "bezerro",
        label: "Bezerras",
        count: getCount("bezerro", "F"),
        categoriaLabel: "bezerra",
        sexo: "F" as const,
      },
    );
  }
    ...
    displayGroups.push(
      {
        profile: "touro",
        label: "Touros",
        count: getCount("touro", "M") + getCount("touro", "U"),
        categoriaLabel: "touro",
        sexo: "M" as const,
      },
      {
        profile: "boi",
        label: "Bois",
        count: getCount("boi", "M") + getCount("boi", "U"),
        categoriaLabel: "boi",
        sexo: "M" as const,
      },
      {
        profile: "novilha",
        label: "Novilhos",
        count: getCount("novilha", "M"),
        categoriaLabel: "novilho",
        sexo: "M" as const,
      },
      {
        profile: "bezerro",
        label: "Bezerros",
        count: getCount("bezerro", "M"),
  ...
  for (const [key, val] of counts.entries()) {
    if (
      key.startsWith("generic_") ||
      (key.endsWith("_U") &&
        !key.startsWith("touro_") &&
        !key.startsWith("boi_"))
    ) {
      generics += val;
    }
