...
export function MobileBottomNav({ onOpenMenu }: MobileBottomNavProps) {
  ...
  const itemClasses = (item: BottomNavigationItem) => {
    ...
    return cn(
      ...
      featured &&
        "mx-auto -mt-6 h-16 w-16 rounded-full border-2 border-primary bg-primary text-primary-foreground shadow-soft hover:bg-primary-hover active:scale-95",
      featured &&
        isActive &&
        "ring-4 ring-primary-subtle ring-offset-2 ring-offset-background",
    );
  };

  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 px-2 pb-[env(safe-area-inset-bottom)] pt-2 shadow-crisp backdrop-blur md:hidden"
