...
export interface CategoriaPayload {
  order?: number;
  criteria?: {
    papel_macho?: 'reprodutor' | 'rufião' | 'corte';
...
export function classificarAnimal(
  ...
  const categoriasOrdenadas = [...categorias].sort((a, b) => {
    ...
    const orderA = (a.payload as CategoriaPayload | undefined)?.order ?? 9999;
    const orderB = (b.payload as CategoriaPayload | undefined)?.order ?? 9999;
  ...
  return (
    categoriasOrdenadas.find((cat) => {
      ...
      const payload = cat.payload as CategoriaPayload | undefined;
