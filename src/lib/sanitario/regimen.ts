...
export interface SanitaryScheduleRule {
  ...
  calendarMode: SanitaryBaseCalendarMode | null;
...
export function inferSanitaryRegimenMilestone(
  ...
  if (!input.scheduleKind && dependsOnMilestone) {
    scheduleKind =
      calendarRule?.mode === "rolling_interval"
        ? "rolling_from_last_completion"
        : "after_previous_completion";
  } else if (
    !input.scheduleKind &&
  ...
  return {
    ...
    schedule_rule: {
      kind: scheduleKind,
      calendarMode: calendarRule?.mode ?? null,
      intervalDays:
    ...
    schedule_rule: {
      kind:
        ...
        readString(scheduleRaw, "kind") === "rolling_from_last_completion"
          ...
          : "calendar_base",
      calendarMode:
        readString(scheduleRaw, "calendar_mode") === "campaign" ||
        readString(scheduleRaw, "calendar_mode") === "age_window" ||
        readString(scheduleRaw, "calendar_mode") === "rolling_interval" ||
        readString(scheduleRaw, "calendar_mode") === "immediate" ||
        readString(scheduleRaw, "calendar_mode") === "clinical_protocol"
          ? (readString(scheduleRaw, "calendar_mode") as SanitaryBaseCalendarMode)
          : null,
