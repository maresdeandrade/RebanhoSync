...
import { normalizeSanitaryFamilyCode } from "@/lib/sanitario/engine/protocolLayers";
...
function normalizeCalendarMode(value: unknown): SanitaryCalendarDraftMode {
  return typeof value === "string" ? fromSqlOrLegacyCalendarMode(value) ?? "" : "";
}

function normalizeCalendarAnchor(value: unknown): SanitaryCalendarDraftAnchor {
  return typeof value === "string" ? fromSqlOrLegacyCalendarAnchor(value) ?? "" : "";
}
...
function inferCalendarLabel(draft: SanitaryProtocolItemDraft): string {
  switch (toSqlCalendarMode(draft.calendarMode)) {
    case "campaign":
...
function buildCalendarPayload(
  ...
  const calendarMode = toSqlCalendarMode(draft.calendarMode);
  const calendarAnchor = toSqlCalendarAnchor(draft.calendarAnchor);

  if (!calendarMode || !calendarAnchor) {
    return null;
  }
...
export function validateProtocolItemDraft(
  ...
  if (
    toSqlCalendarMode(draft.calendarMode) === "campaign" &&
