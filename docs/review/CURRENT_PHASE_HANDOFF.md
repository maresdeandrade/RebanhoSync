# Handoff atual — Fase 22 / F22C.3 qualified occupancy metrics
...
Status: **Fase 22 ativa; F22C.3 duração qualificada, agregação e adoção parcial implementadas**
...
Para 22C, `B4 REMOTE_CONVERGENCE_VERIFIED` está integrado. O [contrato histórico](./F22C_HISTORICAL_OCCUPANCY_SOURCE_GATE.md) mantém `LOT_SOURCE = READY` e `PASTURE_SOURCE = READY`; F22C.1 implementa animal→lote, F22C.2 compõe animal→pasto e F22C.3 calcula duração apenas sobre boundaries demonstráveis, inclusive recorte por período e intervalo aberto com `referenceDate` explícita. A agregação mantém known/unknown, coverage, limitações e conflitos por animal/lote/pasto. `state_*` e última entrada deixaram de ser fallback de permanência nos consumidores migrados; UA/ha, @/ha, GMD e desempenho por pastagem permanecem fora deste contrato.
...
A F22B.2 implementa `calculateObservedEconomicResult` sobre o contrato canônico, sem reler fontes. O [gate F22B.3](./F22B_ADOPTION_PRESENTATION_GATE.md) confirmou que nenhum consumidor atual preserva todas as qualificações sem mudança de UX. Financeiro e Relatórios formam a Wave 2; competência, previsões, comercial e inventário permanecem separados. A F22C.3 não altera esse bloqueio. Novo incremento depende de capacidade de produto explicitamente autorizada.
...
4. **B4 — Convergência de Movimentação:**
   ...
   - Gate F22C: fechado com fontes históricas `READY`; F22C.1 implementa animal→lote, F22C.2 compõe animal→pasto e F22C.3 implementa duração qualificada, agregação e adoção parcial dos consumidores de permanência. Evidências: [contrato F22C](./F22C_HISTORICAL_OCCUPANCY_SOURCE_GATE.md) e [`B4_REMOTE_MOVEMENT_CONVERGENCE_20260829.md`](evidence/B4_REMOTE_MOVEMENT_CONVERGENCE_20260829.md).
