# Handoff atual — Fase 22 / source gate F22C
...
Status: **Fase 22 ativa; source gate F22C fechado com caveats de coverage**
...
Para 22C, `B4 REMOTE_CONVERGENCE_VERIFIED` está integrado. O [source gate histórico](./F22C_HISTORICAL_OCCUPANCY_SOURCE_GATE.md) confirmou `LOT_SOURCE = READY` e `PASTURE_SOURCE = READY`: há fatos datados para animal→lote e lote→pasto, mas cadastro inicial pode deixar `LEFT_BOUND_UNKNOWN` e a composição deve expor coverage/conflitos. Permanecem proibidos o preenchimento por `state_*` e as métricas dias em lote/pasto, UA/ha, @/ha e desempenho por pastagem.
...
4. **B4 — Convergência de Movimentação:**
   ...
   - Gate F22C: fechado com fontes históricas `READY` e caveats de coverage; F22C.1 não iniciado. Evidências: [gate F22C](./F22C_HISTORICAL_OCCUPANCY_SOURCE_GATE.md) e [`B4_REMOTE_MOVEMENT_CONVERGENCE_20260829.md`](evidence/B4_REMOTE_MOVEMENT_CONVERGENCE_20260829.md).
