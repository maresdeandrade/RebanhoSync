...
- Fase 12G — Importador controlado dos Protocolos Sanitarios v2 executada localmente, criando `scripts/codex/import-sanitario-protocols-v2.mjs` com modos `--validate`, `--dry-run` e `--apply`; `--validate` e `--dry-run` passaram, `--apply` ficou bloqueado sem `ALLOW_SANITARIO_IMPORT=1`, members seguem rejeitados e nenhum import real foi executado.
