...
- produto/lote/estoque ja possuem base estrutural tenant-scoped, contratos offline iniciais, UI operacional validada em smoke real local para entradas, ajustes e consumos, edicao de cadastros sem saldo destrutivo, estoque minimo/ponto de ressuprimento por insumo via payload, relatorio operacional com CSV/impressao, demanda futura estimada por agenda valida e alerta operacional de reposicao combinando saldo, parametro e demanda futura sem criar baixa automatica; a Central Operacional/Home tambem expoe esse alerta como sinal passivo/read-only;
