--- supabase/migrations/20260409183000_official_sanitary_catalog_foundation.sql (原始)
begin;
...
+++ supabase/migrations/20260409183000_official_sanitary_catalog_foundation.sql (修改后)
begin;
...
from (
  values
    ...
    (
      ...
      'bezerro',
      'risco',
      jsonb_build_object('risk_field', 'zona_raiva_risco', 'risk_values', jsonb_build_array('medio', 'alto'), 'age_start_days', 90, 'age_end_days', null),
      jsonb_build_object('dose_num', 1),
      false,
      false,
      '{}'::jsonb,
      ...
      jsonb_build_object(
        'produto', 'Vacina Antirrabica',
        'label', 'Raiva - dose inicial em area de risco',
        'indicacao', 'Aplicar a partir dos 90 dias de idade (3 meses) em fazendas com risco configurado, conforme recomendacoes do MAPA.',
        'dedup_template', 'raiva:{animal_id}:d1'
      )
    ),
    (
      ...
      'bezerro',
      'risco',
      jsonb_build_object('risk_field', 'zona_raiva_risco', 'risk_values', jsonb_build_array('medio', 'alto'), 'age_start_days', null, 'age_end_days', null),
      jsonb_build_object('dose_num', 2, 'interval_days', 30),
      false,
      false,
      '{}'::jsonb,
      ...
      jsonb_build_object(
        'produto', 'Vacina Antirrabica',
        'label', 'Raiva - reforco apos 30 dias',
        'indicacao', 'Segunda dose da primovacinacao, aplicada 30 dias apos a primeira dose, conforme recomendacoes do MAPA.',
        'dedup_template', 'raiva:{animal_id}:d2'
      )
    ),
    (
      ...
      jsonb_build_object(
        'produto', 'Vacina Antirrabica',
        'label', 'Raiva - revacinacao anual',
        'indicacao', 'Revacinacao anual apos o ciclo inicial (dose 1 + reforco 30 dias), conforme recomendacoes do MAPA.',
        'dedup_template', 'raiva:{animal_id}:anual'
      )
    ),
    (
