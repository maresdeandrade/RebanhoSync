...
describe("sync-batch rules: narrowing reprodutivo e erro interno", () => {
  it("normaliza payload ausente, não objeto e episode_evento_id inválido", () => {
    for (const value of [undefined, null, "payload", [], 42]) {
      const normalized = readReproductionPayload(value);
      expect(normalized).toEqual({
        reproductionPayload: {},
        schemaVersion: undefined,
        episodeEventId: null,
        episodeLinkMethod: null,
      });
    ...
    expect(
      readReproductionPayload({
        schema_version: 1,
        episode_evento_id: 42,
        episode_link_method: false,
      }),
    ).toMatchObject({
      schemaVersion: 1,
      episodeEventId: null,
  ...
  it("normaliza relação Supabase como array, objeto ou ausente", () => {
    expect(readLinkedReproductionType([{ tipo: "cobertura" }])).toBe(
      "cobertura",
    );
    expect(readLinkedReproductionType({ tipo: "IA" })).toBe("IA");
    expect(readLinkedReproductionType(undefined)).toBeNull();
    expect(readLinkedReproductionType([])).toBeNull();
    expect(readLinkedReproductionType({ tipo: 42 })).toBeNull();
  });

  it("atribui episode_link_method sem perder o payload normalizado", () => {
    const { reproductionPayload, episodeLinkMethod } = readReproductionPayload({
      schema_version: 1,
      note: "preserve",
    });
  ...
  it("preserva rawOp.client_op_id quando uma exceção interna é tratada", () => {
    const rawOp = op({ client_op_id: "op-internal" });
    let result: ReturnType<typeof buildInternalErrorResult> | undefined;

    try {
      throw new Error("forced internal failure");
    } catch (error: unknown) {
      result = buildInternalErrorResult(rawOp, error);
    }
...
describe("sync-batch rules: agenda source inference", () => {
  it("uses direct source_task_id on event insert", () => {
    ...
    expect(inferAgendaSourceTaskIdForEventInsert(operation, [operation])).toBe(
      "agenda-1",
    );
  });
  ...
  it("infers agenda source from same-batch agenda completion update", () => {
    ...
    expect(inferAgendaSourceTaskIdForEventInsert(eventOp, [eventOp, agendaOp]))
      .toBe(
        "agenda-1",
      );
  });
