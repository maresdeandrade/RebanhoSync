...
export function classifyDeltaScope(files, known = true) {
  ...
  const executableFiles = normalized.filter((file) => !isDocsFile(file));
  if (executableFiles.length > 0 && executableFiles.every(isTestFile)) return "TEST_ONLY";
  if (normalized.some(isFullFile)) return "FULL";
  return "APP";
}
...
function testDirectoryTarget(file) {
  ...
  if (testsIndex >= 0) return segments.slice(0, testsIndex + 1).join("/");
  if (segments[0] === "tests") return segments.slice(0, Math.min(2, segments.length - 1)).join("/");
  return null;
}
...
function runCli(args) {
  ...
  if (command === "test-targets") {
    const [file] = rest;
    for (const target of selectTestTargets(readFileList(file))) console.log(target);
    return;
  }

  if (command !== "classify") throw new Error("Expected command: classify or test-targets");
  ...
  const deltaScope = finalGate
    ? "FULL"
    : classifyDeltaScope(readFileList(options["delta-files"]), toBoolean(options["delta-known"], false));
  const execution = deriveExecution({ prScope, deltaScope, finalGate });
