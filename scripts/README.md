...
- `antigravity/validate_f24_1_scope.sh`: allowlist exata e local-worktree do closeout versionado F24.1.
...
- `pnpm run gates:f24-1-scope`: gate histórico local-worktree específico do closeout F24.1.
- `pnpm run audit:agents`: auditoria estática de `.agents`.
- `gates:scope`: alias estável de `gates:docs-scope`; não substitui revisão do diff commitado do PR.
